...
- Keep live text, logos/vectors, and photographs separate, with meaningful object or layer names.
...
- Retain enough image detail for the intended layout. Flag an inadequate original rather than claim that changing DPI metadata restores it.
...
Export a preview from the actual editable source and inspect it. Reasoning about source coordinates alone is insufficient.
...
Deliver the editable source, appropriate review previews, and requested digital exports. A PDF produced here is a design/review export pending technical print checks. Include a short project note with chosen fonts/assets, unresolved items, and intended dimensions/variants for the later print-preflight step. If that skill is unavailable, state that status and still deliver completed design work.
